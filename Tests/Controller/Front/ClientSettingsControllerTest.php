 ...
 */
class ClientSettingsControllerTest extends AbstractFrontControllerTestCase
{
    public function testIndexLoggedAction()
    {
        $this->logIn();
        
        $url     = $this->generateUrl('front.client_settings.index');
        $crawler = $this->client->request('GET', $url);
        
        $this->assertTrue($this->client->getResponse()->isSuccessful());
        $this->assertEquals(1, $crawler->filter('html:contains("' . $this->trans('client.heading.contact_details') . '")')->count());
